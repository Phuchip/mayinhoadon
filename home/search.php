...
if ($id != 0) {
    $query_check = new db_query("SELECT * FROM `tbl_tags` WHERE `id` = ".$id);
    if(mysql_num_rows($query_check->result) > 0){
        $row = mysql_fetch_assoc($query_check->result);
        $link = $row['alias'].'-ID-'.$row['id'].'.html';
        if($url != $link){
        ...
        $sql = new db_query("SELECT id,name,alias,price_old,discount,quantity,image,code_product,contact,rate,total_rate FROM `tbl_product` WHERE `delete` = 0 AND `status` = 1 AND FIND_IN_SET('" . $id . "',tag) " . $where . " OR `name` LIKE '%".$_SESSION['search']."%' OR `review_product` LIKE '%".$_SESSION['search']."%' ORDER BY `tag` DESC, " . $order . " modify_time DESC " . $pagination);
        $query_total = new db_query("SELECT id,name,alias,price_old,discount,quantity,image,code_product,contact,rate,total_rate FROM `tbl_product` WHERE `delete` = 0 AND `status` = 1 AND FIND_IN_SET('" . $id . "',tag) " . $where . " ");
    }else{
        header("HTTP/1.1 301 Moved Permanently"); 
        header("Location: 404"); 
        exit();
    ...
    $sql = new db_query("SELECT id,name,alias,price_old,discount,quantity,image,code_product,contact,rate,total_rate FROM `tbl_product` WHERE `delete` = 0 AND `status` = 1 AND `name` LIKE '%" . $key . "%' " . $where . " ORDER BY " . $order . " modify_time DESC " . $pagination);
    $query_total = new db_query("SELECT id,name,alias,price_old,discount,quantity,image,code_product,contact,rate,total_rate FROM `tbl_product` WHERE `delete` = 0 AND `status` = 1 AND `name` LIKE '%" . $key . "%' " . $where . "");
}
$manu = explode(',', $manu);
$cate = explode(',', $cate);
...
<body>
    <div class="container-fluid main-container">
        ...
        <div class="body-page pd0">
            ...
            <!--        filter-->
            <div class="row mt-5 ft1 filter_page">
                <!-- filter right-->
            ...
            <!--        filter end-->
            <!--        top product-->
            <p class="text-uppercase mt-3 text-decoration-underline top-cate">Tìm kiếm : <?= $_SESSION['search'] ?></p>
            <div class="row mt-4">
                <?php
