...
function makeML($content, $search, $replace, $amp = '')
{
    if ($content != '') {
        ...
        $img = ($amp != '') ? '<amp-img width=40 height=40 layout="intrinsic" src="/images/img_light.png"></amp-img>' : "<img style='width:33px;height:41px' src='/images/load.gif' class='lazyload' data-src='/images/img_light.png' />";
...
function makeMLBlog($content, $search, $replace)
{
    if ($content != '') {
        ...
        $ml = "<nav role='navigation' class='table-of-contents'>
          <div class='box_title_pl'>
            <div class='icon'><img src='/images/load.gif' class='lazyload' data-src='/images/img_light.png'></div>
            <p class='tt_phu_luc'><span>Mục lục</span></p>
          </div>
          <ul>";
        $i = $u = $j = 0;
...
function amp_content($string)
{
    ...
    foreach ($dom->getElementsByTagName('a') as $a) {
        ...
        if (in_array(end(explode('.', $href)), array('docx', 'doc', 'pdf', 'xlsx', 'xls', 'rar', 'zip'))) {
            $a->setAttribute("class", "download_365");
            $a->nodeValue = 'Tải xuống ngay';
        }
    }
...
function makeML_content_amp($content, $search = '', $replace = '')
{
    if ($content != '') {
        $html = str_get_html($content);
        $h2s = $html->find("h2,h3,h4");
...
function check_exits_tv($idkey, $city)
{
    $check_exits = new db_query("SELECT count(*) FROM tv_key_city WHERE id_key = $idkey AND id_city = $city LIMIT 1");
    $r_exits = mysql_fetch_assoc($check_exits->result);
    $exits_count = $r_exits['count(*)'];
    if ($exits_count == 0) {
...
function makeMLBlog_amp($content, $search, $replace, $amp = '')
{
    if ($content != '') {
        require_once("../functions/simple_html_dom.php");
        $html = str_get_html($content);
        $h2s = $html->find("h2,h3,h4");
        $img = ($amp != '') ? '<amp-img width=33 height=41 layout="intrinsic" src="/images/img_light.png"></amp-img>' : "<img src='/images/load.gif' class='lazyload' data-src='/images/img_light.png' />";
        $patterns = array('/\d+\.\d+\.\d+\.\s/i', '/\d+\.\d+\.\s/i', '/\d+\.\s/i');
        $ml = "<nav role='navigation' class='table-of-contents'>
          <div class='box_title_pl'>
            <div class='icon'>" . $img . "</div>
...
function api_hide_cv($id, $link)
{
    $new_array_cv = array(
        'id' => $id,
        'link' => str_replace('../', "https://timviec365.com" . '/', $link)
    );
    $array_cv[] = $new_array_cv;
    $arr_cvuv = json_encode($array_cv, JSON_UNESCAPED_UNICODE);
    try {
        $i = 0;
        $curl = curl_init();
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS, $arr_cvuv);
        // curl_setopt($curl, CURLOPT_PROXY, "http://103.138.113.107:8000/"); //your proxy url
        // curl_setopt($curl, CURLOPT_PROXYPORT, "8000"); // your proxy port number
        curl_setopt($curl, CURLOPT_URL, 'https://timviec365.vn/api/tmp_curl_tv365com.php');
        ...
        curl_close($curl);
        $json_res = substr($response, strpos($response, "["));
        $arr_res = json_decode($json_res);

        if (count($arr_res) > 0) {
            foreach ($arr_res as $key => $value) {
                $data = [
                    'link_scan' => $value->link,
                    'scanned' => 1
                ];
                $condition = [
                    'id_upload' => $value->id
                ];
                update('user_cv_upload', $data, $condition);
            }
        }
    } catch (Exception $e) {
...
function save_image_compress($url, $base_64)
{
    $url = str_replace('https://timviec365.com', '..', $url);
    $result = false;
    if (is_file($url) && $base_64 != '') {
        $explode = explode('/', $url);
        $path = [];
        ...
        $path = implode('/', $path);
        $explode_name = explode('.', array_pop($explode));
        $old_name = $explode_name[0];
        $type = $explode_name[1];
        $new_name = $old_name . "_compress" . ".{$type}";
        $full_new_name = $path . "/" . $new_name;
        $b64_decode = base64_decode($base_64);
        if (file_put_contents($full_new_name, $b64_decode) > 0) {
            unlink($url);
            $result = $new_name;
        }
    }
...
function redirect301_freelancer($category, $city, $cit_name)
{
    $url = '';
    $array = [1, 2, 45, 26, 48];
    if ($category == 67 && $city == 0) {
        $url = "https://freelancer.timviec365.com/";
    }
    if ($category == 67 && in_array($city, $array)) {
        $url = "https://freelancer.timviec365.com/tim-viec-lam-freelancer-tai-" . replaceTitle($cit_name) . "-flt" . $city . ".html";
    }
    if ($url != '') {
        header("HTTP/1.1 301 Moved Permanently");
        header("Location: $url");
