...
if($action == ''){
    $sql = new db_query("SELECT id,name,modify_time,status FROM `tbl_category` WHERE 1 ORDER BY modify_time DESC");
}
role($role,$action,$id);
if($action == 'delete' AND $id != 0){
    $sql = new db_query("DELETE FROM `tbl_category` WHERE `id` = ".$id);
    echo '
    <script language="javascript">
    window.alert("Xóa thành công");
   	window.parent.location.href="./loai-may-in";
   </script>';
}
if($action == 'add' AND $id != 0){
    $sql = new db_query("SELECT * FROM `tbl_category` WHERE 1 AND id = ".$id);
}
if($action == 'add' AND $submit != ''){
    ...
    if($submit == 'add'){
        add('tbl_category',$data);
        echo '
        <script language="javascript">
            window.alert("Thêm thành công");
        </script>';
    }elseif($submit == 'update'){
        update('tbl_category',$data,$condition);
        echo '
        <script language="javascript">
