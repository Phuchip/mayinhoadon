...
if($action == ''){
    $sql = new db_query("SELECT id,name,modify_time,status FROM `tbl_paper_size` WHERE 1 ORDER BY modify_time DESC");
}
role($role,$action,$id);
if($action == 'delete' AND $id != 0){
    $sql = new db_query("DELETE FROM `tbl_paper_size` WHERE `id` = ".$id);
    echo '
    <script language="javascript">
    window.alert("Xóa thành công");
   	window.parent.location.href="./kho-giay";
   </script>';
}
if($action == 'add' AND $id != 0){
    $sql = new db_query("SELECT * FROM `tbl_paper_size` WHERE 1 AND id = ".$id);
}
if($action == 'add' AND $submit != ''){
    ...
    if($submit == 'add'){
        add('tbl_paper_size',$data);
        echo '
        <script language="javascript">
            window.alert("Thêm thành công");
        </script>';
    }elseif($submit == 'update'){
        update('tbl_paper_size',$data,$condition);
        echo '
        <script language="javascript">
